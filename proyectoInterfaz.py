...
class Example(QWidget):
    ...
    def btnCambiosClicked(self,event):
        ...
        for i in data['events']:
            print(i)
            print(" ")
            if self.targetAsn == i['attrs']['path'][0]:  
                #print(i)   
                eventos.append(i)
                self.eventsInAs += 1
        ...
        if self.contador == 0:
            ...
            for i in self.data['initial_state']:
                ...
                if self.targetAsn in i['path']:
                    ...
                    G.add_nodes(i['path'], label=etiquetas)
           ...
           # print(evento['timestamp'])            
            ...
            G.add_nodes(evento['attrs']['path'], label=etiquetas)
            for j in range(1,len(evento['attrs']['path'])):     
                G.add_edge(evento['attrs']['path'][j],evento['attrs']['path'][j-1],color='green')
